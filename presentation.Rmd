...
<aside class="notes">
We'll start with an orders table. Each row is an order that contains the order number, the product that was sold, a customer id, the date, and the dollar amount. The goal here is to get information on sales over time for each product. So we basically need to sum dollar amount by date.
</aside>
...
<aside class="notes">
Showing the data by day will be too granular, so we want to get it into months. We can add in some DAX code to create a new column for the month of each order. We'll also just use date as a date hierarchy and compare the methods.
</aside>
...
<aside class="notes">
This graph uses the month column we just created. You can see there are some missing values at the start and end of the dark blue line. The orange line is suspiciously straight.
</aside>
...
<aside class="notes">
This graph uses the date column from the data at the month level. You still see the missing values for the dark blue line, but now the orange line has missing values. The other graph was just connecting the segments across January to November.
</aside>
...
<aside class="notes">
At a close look at the graph you see that there are three products that sale at different time of the year. Product A sells year round. Product B is only in the summer while product C is only in the winter. 
</aside>
...
<aside class="notes">
What we really want to see are all the lines filled in down to zero. The poorly drawn in lines display what we want to get at the end.
</aside>
...
<aside class="notes">
We can also see that this can mess up any calculations. In this example, the monthly_average is wrong because distinct_months is incorrect for selling over a full year.
</aside>
...
<aside class="notes">
The next section will cover some basic strategies to fill in zeros for missing values. These techniques will handle a lot of problems and are pretty easy to implement.
</aside>
...
<aside class="notes">
If you click on the down arrow on your axis, there will be an option for 'Show items with no data'. This is always the first option I try. It only works in some situations and doesn't work in this example.
</aside>
...
<aside class="notes">
We can just fix the math if you know it's a constant. All we need to do is add 0 to the end of our measure and it will fill in for us. Unfortunately, this only fixes the graph and not the table.
</aside>
...
<aside class="notes">
We can fix the table by skipping the DISTINCTCOUNT on months and replace it all with 12. This will get the correct monthly_average. However, this only work when we know it's always 12. Also, it doesn't fix the graph.
</aside>
...
<aside class="notes">
For these strategies, they tend to work very easily. None of them took a lot of effort to implement from where we started. However, they tend to not be extendable. We have to mix and match these strategies to get everything, but that's annoying. They can fall apart for more advance calculations and become messy when trying to keep it all together. In my experience, if you just need basic graphs and tables, these are great. If you know you'll more advance calculations and interactivity, a more thorough setup can be better.
</aside>

# Better Strategy {.section_slide .center}
<aside class="notes">
This next section goes through that more thorough setup. We'll reset everything and build a stronger start to enable us to get any calculations we need.
</aside>
...
<aside class="notes">
There's still a year of data but a lot more products. This time there are other trends including decreasing or increasing over time and even products that only sold for one month (those are the points). We'll call this orders_big.
</aside>
...
<aside class="notes">
This is just a reminder of what we're looking to create by the end.
</aside>
...
<aside class="notes">
There will be three parts to this one. The first part is setting up a calendar table to use as a date dimension.
</aside>
...
<aside class="notes">
A calendar table is a table created in Power BI that contains all the dates you need and associated information. We'll use the Calendar Dax function. This function needs start and end dates. It'll then fill out all the dates in between (including the start and end dates). We'll go through three examples.
</aside>
...
<aside class="notes">
We have three different ways to create a date table using the CALENDAR function. The first way provides specific dates. This is useful if you ever need the graph to keep the same date range even if your data changes. The second way uses the TODAY function to get the current date. This will keep your dashboard current no matter what happens with your data. The last way pulls the start and end dates from the data. This will keep your graphs matching the data even if it doesn't refresh. I tend to use either of the last two depending on the business need.

There are other options like CALENDARAUTO or setting a date table, but for this presentation we'll stick with CALENDAR. It's definitely worth your time to spend an afternoon looking into the other options if you're going to implement this at your work.
</aside>
...
<aside class="notes">
Since we'll want to group by month, we'll add in a month column. We'll also change format to match our data. You typically always want it to match as this can speed up calculations.
</aside>
...
<aside class="notes">
The star schema will work by placing the main orders_big table in the middle. Then have important dimensions on the outside, like our date table. Often you'll see this has a 'suggestion' from Microsoft, but it's almost always a good idea from the beginning.
</aside>
...
<aside class="notes">
Putting it all into the ADDMISSINGITEMS function, you can see ShowAll_ColumnName cal_month and product, the Table is the SUMMARIZECOLUMNS, the GroupBy_ColumnName is the same as the ShowAll_ColumnName, no filters for now. This is basically a full join between calendar months and products, then a left join to our summarized data. We can see some of the missing months in the image.
</aside>
...
<aside class="notes">
I like to add in amount filled, but you could replace amount if you wanted.
</aside>
...
<aside class="notes">
If you have a lot of data you might get a warning in the top corner about not seeing all the data. There's an option that samples the data to increase speed. You can turn that off to see more lines. There's not a strict cutoff on data size. It just depends on what you're graphing.
</aside>
...
<aside class="notes">
Let's say different products were available for months.
</aside>
...
<aside class="notes">
There are multiple rows per product for cases when there are jumps in the dates. So you can't just join to orders_big.
</aside>
...
<aside class="notes">
Build table with month filled down. (There are others ways to do this, especially when bringing the data into Power BI)
</aside>
...
<aside class="notes">
Still not quite right. Notice the orange line across the middle.
</aside>
...
<aside class="notes">
Go back before we filtered on availability.
</aside>
...
<aside class="notes">
Create a new star schema. Can't reuse the same tables.
</aside>
...
<aside class="notes">
Need to watch this in calculations like the one we just did.
</aside>
...
<aside class="notes">
Just need to add DISTINCTCOUNT of customer_id and carry through.
</aside>
